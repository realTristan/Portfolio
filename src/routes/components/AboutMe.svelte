<script lang="ts">
  ...
  let summary1Words: any[] = setSummaryData(summary1.split(" "), [
...
<div
  use:viewport
  class="{headerInView ? 'opacity-1 -translate-y-8': 'opacity-0 -translate-y-0'} duration-[2000ms] ease-in-out"
  on:enterViewport={() => {
    ...
    for (let i = 0; i < summary1Words.length; i++) {
      setTimeout(
        () => (summary1Words[i].visible = true),
        summary1Words[i].delay + 1000
      );
    }
    for (let i = 0; i < summary2Words.length; i++) {
      setTimeout(
        () => (summary2Words[i].visible = true),
        summary2Words[i].delay + 11200
      );
    }
  }}
>
